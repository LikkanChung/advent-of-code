...
def get_key_from_mapping(destination: int, maps: list):
    for map in maps:
        if destination >= map.get("destination") and destination <= map.get("destination") + map.get("range"):
             offset = destination - map.get("destination")
             return map.get("source").get("start") + offset
    return None


def get_locations(lines: list):
    start_range = {}
    min_loc = []
    for line in lines:
          ...
          min_loc.append(destination)
          start_range[destination] = range_length
    min_loc.sort()
    return start_range, min_loc
...
with open(input_vals.get("file"), "r") as fd:
    ...
    location_ranges, ordered_locations = get_locations(lines[input_vals.get("humidity_location").get("start"): input_vals.get("humidity_location").get("end")])

    print(ordered_locations)

    for d in ordered_locations:
        for r in range(location_ranges.get(d)):
             location = d + r
             print("checking " + str(location))

             humidity = get_key_from_mapping(location, humidity_location_map)
